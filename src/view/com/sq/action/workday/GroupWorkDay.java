...
public class GroupWorkDay {
	...
	public String workDayExport(String fmlFileName , String filePath , SqDocmentManager sqDocmentManager) {
		ComThread.InitSTA();
		int tableIndex = 1 ;
		WordBean msWordManager = new WordBean();
		WordBean msWordManager1 = new WordBean();
		String fileName = this.getGroupName() + "周报"+this.getStartDate()+"到"+this.getEndDate()+"("+this.getUserName()+").doc";
		try {
			//项目组周报
			...
			//编写日期
			msWordManager1.replaceAllText("$DATE$", this.getEndDate());
			int tempCount = 2 ;
			//周报工作内容
			...
			//工作建议
			tempCount = 2+tempCount+this.getDiscussProblemList().size() ;
			...
			String content = msWordManager1.getContent(tableIndex, tempCount, 2);
			...
			//如果有周例会，则拷贝一份周例会到周报中去
			if(sqDocmentManager !=null){
				String docPath = ServletActionContext.getServletContext().getRealPath(File.separator) + sqDocmentManager.getFilePath() + sqDocmentManager.getFileName();
				msWordManager1.moveDown(8);
				msWordManager1.insertText("$abc$");
				msWordManager1.moveUp(3);
				msWordManager1.copyTableFromAnotherDoc(docPath, 1, "$abc$");
			}
